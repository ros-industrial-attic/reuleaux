...
ros::init(argc, argv, "inverse_workspace");
  ...
  while (ros::ok())
  {
    ...
    h5file.open();
    h5file.loadMapsFromDataset(PoseColFilter, SphereCol, res);
      ...
      if (pointIdxVec.size() > 0)
      {
        ...
        for (size_t j = 0; j < pointIdxVec.size(); ++j)
        {
          ...
          for (it1 = trns_col.lower_bound(base_pos); it1 != trns_col.upper_bound(base_pos); ++it1)
          {
            ...
            baseTrnsCol.insert(std::make_pair(base_sphere, base_pose));
          }
        }
       }
    }
   ...
   Map sphereColor;
   for (MultiMap::iterator it = baseTrnsCol.begin(); it!=baseTrnsCol.end(); ++it)
   {
     const std::vector<double>* sphere_coord = it->first;
     float d = (float(baseTrnsCol.count(sphere_coord)) / pose.size()) * 100;
   ...
   irm_h5.saveReachMapsToDataset(baseTrnsCol, sphereColor, res);
