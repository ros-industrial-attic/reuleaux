...
//Starting to create the Inverse Reachability map. The resolution will be the same as the reachability map
   ...
   int numofFinalPose = 10;//How many optimal base pose we need (But each pose is from a single sphere)
	...
	//cout<<ws.WsSpheres[i].poses.size()<<endl;
        findOptimalPosebyPCA(ws.WsSpheres[i].poses,final_base_pose);//Calling the averaging function 
