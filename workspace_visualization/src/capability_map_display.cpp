...
CapMapDisplay::CapMapDisplay()
{
  ...
  disect_property_ = new rviz::EnumProperty( "Disect", "Full", "Disection of the workspace", this,  SLOT( updateColorAndAlpha()));
  ...
  color_property_ = new rviz::ColorProperty( "Color", QColor( 255, 225, 102 ), "Color to draw the Sphere.", this, SLOT( updateColorAndAlpha() ));
...
void CapMapDisplay::processMessage( const map_creator::capability::ConstPtr& msg )
{
  ...
  visual->setMessage( msg, lower_bound_reachability_->getInt(), upper_bound_reachability_->getInt(), disect_property_->getOptionInt()  );
