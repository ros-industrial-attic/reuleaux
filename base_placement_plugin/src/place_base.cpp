...
{
  /*! 
      Once the initialization of the RViz is has finished, this function sends the pose of the initial marker. It also sends the method list and the visualization method lists.
  */
...
void PlaceBase::setBasePlaceParams(int base_loc_size, int high_score_sp)
{
  /*! Set the necessary parameters for the Base Placement process.
      These parameters correspond to the ones that the user has entered or the default ones before pressing the find base button.
...
void PlaceBase::findbase(std::vector<geometry_msgs::Pose> grasp_poses)
{
    ...
    */
    ...
    {
      tf2::Quaternion quat(final_base_poses[i].orientation.x, final_base_poses[i].orientation.y, final_base_poses[i].orientation.z, final_base_poses[i].orientation.w);
...
{
    /* PCA Method: The planner takes desired number of high scoring spheres and implements PCA for finding optimal orientations from all the poses correspond to that sphere. One pose from one sphere.
    */
...
void PlaceBase::findBaseByIKSolutionScore()
{
    ...
    */
    ...
    vector<geometry_msgs::Pose> final_base_loc;
    for (multiset<pair<double, vector<double> > > ::iterator it = basePoseWithHits.begin();it != basePoseWithHits.end();++it)
    {
      geometry_msgs::Pose final_base;
      sd.convertVectorToPose(it->second, final_base);
      final_base_loc.push_back(final_base);
...
{   
    ...
    */
    ...
    for (int i=0;i<po.size();++i)
    {
      ...
      //ROS_INFO("received msg [%f] [%f] [%f] [%f] [%f] [%f] [%f]",po[i].position.x,po[i].position.y,po[i].position.z,  po[i].orientation.x,po[i].orientation.y, po[i].orientation.z,po[i].orientation.w);
...
void PlaceBase::showBaseLocationsbyRobotModel(vector<geometry_msgs::Pose> po)
{
    ...
    Robotmodel in rviz is a child of display class. While taking data from a topic can be implemented by messageFilterDisplay class.
...
void PlaceBase::setReachabilityData(std::multimap<std::vector<double>, std::vector<double> > PoseCollection, std::multimap<std::vector<double>, double > SphereCollection, float resolution)
{
...
{
    /* Slot for showing Union map. Can only be visualized after calling the find base function. Otherwise it will show error message
    */ 
